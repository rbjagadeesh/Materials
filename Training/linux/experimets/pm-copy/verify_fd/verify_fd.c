...
/*	printk(KERN_INFO "comm string is %s\n", my_task->comm);
	...
	printk(KERN_INFO "starting address of vm_area_struct %p\n",(void *) my_task -> mm -> mmap ->  vm_start);

	printk(KERN_INFO "ending address of vm_area_struct %p\n", ( void *)my_task -> mm -> mmap ->  vm_end);

	printk(KERN_INFO "starting address of vm_area_struct %p\n",(void *) my_task -> mm -> mmap ->  vm_);
*/

	printk (KERN_INFO "fd is %p\n", my_task->files->fdt->fd[n]);
	printk("\n");
	return 0;
}
