...
long myioctl(struct file *fp, unsigned int pid, unsigned long gvar)
{
	...
	unsigned long *pt_va;
	unsigned int index;
	unsigned long *va;//va for virtual address and pa for physical address
	extern struct page *mem_map;//pointer to descriptors of page frames
	struct task_struct *my_task = current;	
	pgd_t *mypgd = my_task -> mm -> pgd;	//PGD base virtual address

	pgd_va = (unsigned long *)(mypgd + (gvar >> 22));  //va for pgd index

	if((*pgd_va >= LOWMEM_MIN) && (*pgd_va < LOWMEM_MAX)) {

		pgd_va = (unsigned long *)((((*pgd_va) >> OFFSET)<< OFFSET) + PAGEOFFSET);	//add 0xc000_0000 to address since pa in FCOM

	} else {

		pgd_va = (unsigned long *)kmap(mem_map + ((*pgd_va) >> OFFSET));
	//pa in DAMR so use kmap
	}

	index = ((gvar << 10) >> 22);
	pt_va = (unsigned long *) (pgd_va + index);

	if((*pt_va >= LOWMEM_MIN) && (*pt_va < LOWMEM_MAX)) {

		pt_va = (unsigned long *)(((*pt_va >> OFFSET)<<OFFSET)+PAGEOFFSET);

	} else {

		pt_va = (unsigned long *) kmap(mem_map + ((*pt_va) >> OFFSET)); 
	}	

	index = ((gvar << 20) >> 20);
	va = (unsigned long *)((char *)pt_va + index);	

	*va = 125;			//changing value in address

	printk(KERN_INFO "*va %ld\n", *va);
