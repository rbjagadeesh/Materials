...
long myioctl(struct file *fp, unsigned int pid, unsigned long num )
{
	...
	for(i =0; i < 1024; i++) {		//for all entries in pgd
		if(mypgd[i].pgd) {			
			printk(KERN_INFO "pgd[%4d] PGD %08lx\n", i, mypgd[i].pgd);

			if((mypgd[i].pgd >= LOWMEM_MIN) && (mypgd[i].pgd < LOWMEM_MAX)) {

				j++;			//for low mem entries

			} else {

				k++;			//for high mem entries
			}
		}
	}

	printk (KERN_INFO "total entries in PGD %d\n", k + j);
	printk (KERN_INFO "LOW_MEM %d\n", j);
	printk (KERN_INFO "HIGH_MEM %d\n", k);
