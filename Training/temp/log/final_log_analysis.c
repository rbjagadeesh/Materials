...
/*  SrcSystem structure for each system
	log[] array is used to count msgs based on logid that range from 0 - 30
	0 th for Incorrect log msgs for particular system
	1 - 30 th for respective logids(1-30)
*/
struct SrcSystem {
	char *sys_name;
	int log[MAX_LOG];
...
static FILE *validate_input_file(char *ptr)
{
	FILE *fp;
	char *str = ptr;
...
static int parse_log_msg (char *str)
{
	char *ptr;
	int num;
	...
	while (NULL != ptr) {			//for checking incorrect message length
...
static void update_log_count(char *sys, int logid_number)
{
	int i;
...
static void print_master_summary()
{
	int i = 0;
	int total = 0;
	struct SrcSystem *temp = NULL;
...
static void print_Invalid_msg_summary(int n)
{
...
static void print_individual_system_summary()
{
	int i = 0;	
	int total = 0;
	struct SrcSystem *temp = NULL;
